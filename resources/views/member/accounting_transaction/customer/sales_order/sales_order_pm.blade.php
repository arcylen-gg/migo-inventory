	    ...
	    <div class="col-sm-12">
			<table class="digima-table" style="width: 100%">
				...
				<tbody class="draggable-pm">
					@if(isset($sales_order_pm))
						@foreach($sales_order_pm as $key => $_pm)
							<tr class="tr-pm-row">
								<td class="td-pm-id">1</td>
								<td>
								    <select class="form-control drop-down-payment" name="txn_payment_method[]" >
								        @include("member.load_ajax_data.load_payment_method", ['payment_method_id' => $_pm->est_pm_id])
								    </select>
								</td>
								<td >
						    		<input type="text" class="form-control rcvpymnt-refno" name="txn_ref_no[]" id="rcvpymnt-refno" value="{{$_pm->estimate_reference_num or ''}}" />		
								</td>
								<td>
								    <input type="text" class="form-control  text-right number-input-pm" name="txn_payment_amount[]" id="rcvpymnt-refno" value="{{$_pm->estimate_amount or '0.00'}}" />				
								</td>
