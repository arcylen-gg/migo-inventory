<div class="panel panel-default panel-block panel-title-block">
    <div class="panel-heading">
        ...
        <div>
            <div class="col-md-2">
                <select class="form-control input-sm report_week_year" name="report_week_year">
                    @foreach($_year as $year)
                    <option value="{{$year}}" {{$current_year == $year ? 'selected' : ''}}>Year {{$year}}</option>
                    @endforeach
                </select>
                <input type="text" class="form-control from_report_a datepicker hidden" name="from" placeholder="Start Date">
            </div>
            <div class="col-md-2">
                <select class="form-control input-sm report_period hidden" name="report_period">
                ...
                <select class="form-control input-sm report_week_no" name="report_week_no">
                    @foreach($_week as $week)
                    <option value="{{$week}}" {{$current_week == $week ? 'selected' : ''}}>Week {{$week}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2">
                <input type="text" class="form-control form_report_b datepicker hidden" name="to" placeholder="End Date">
            </div>
