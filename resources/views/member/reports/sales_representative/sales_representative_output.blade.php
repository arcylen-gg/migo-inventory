         ...
         <div class="table-reponsive">
              ...
              <th class="text-center" class="text-center">Sales Reprensentative</th>
                ...
                <td><b>{{$salesrep->sales_rep_employee_number}}</b></td>
         				<td><b>{{$salesrep->sales_rep_first_name." ".$salesrep->sales_rep_middle_name." ".$salesrep->sales_rep_last_name}}</b></td>
                <td colspan="3"></td>
                <td class="text-right"><text class="total-report">{{currency('PHP', $salesrep->sales_amount)}}</text></td>
         			</tr>
                ...
                <td nowrap colspan="2"></td>
  							...
  							<td nowrap>{{$sale->company != '' ?  $sale->company : $sale->first_name .' '. $sale->last_name}}</td>
  							<td nowrap>{{date('F d, Y',strtotime($sale->inv_date))}}</td>
  							<td nowrap>{{currency('PHP',$sale->inv_overall_price)}}</td>
  						</tr>
    					@endforeach
      				<tr  data-id="customer2-{{$key}}" data-parent="customer-{{$key}}">
      					<td colspan="5"><b>Total {{$salesrep->sales_rep_first_name." ".$salesrep->sales_rep_middle_name." ".$salesrep->sales_rep_last_name}}</b></td>
