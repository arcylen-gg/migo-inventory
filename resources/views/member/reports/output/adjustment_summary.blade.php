				...
				<table class="table table-condensed collaptable">
					<thead style="text-transform: uppercase">
						<tr>
							...
							<th class="text-center">Total Qty In</th>
                            <th class="text-center">Total Qty Out</th>
							...
							@foreach($_report as $key => $report)
								<tr>
									<td class="text-center">
										<a href="/member/report/adjustment/detailed/{{$report['item_id']}}" >{{$report['item_name']}}</a><br>
										<small>{{$report['item_sku']}}</small><br>
									</td>
									<td class="text-center">{{$report['qty_in']->qty_in ? $report['qty_in']->qty_in : 0}}</td>
                                    <td class="text-center">{{$report['qty_out']->qty_out ? abs($report['qty_out']->qty_out) : 0}}</td>
