...
function customer() {
    ...
    function _ready() {
        ...
        outsideclick();
        popover();
        event_click_add_proposal();
        action_change_budget_price();
        action_check_same_as_bill();        
    }
    function action_change_budget_price()
    {
    	$("body").on("change",".txt-budget-price", function()
    	{
    		$thisval = action_return_to_number($(this).val());
    		$(this).val(action_add_comma($thisval.toFixed(2))).change();
    	});
    }
    ...
    function event_click_add_proposal()
    {
    	$("body").on("click",".btn-add-line", function()
		{
			$(".proposal-div").append($(".div-script-proposal .body-script-proposal").html());
		});
		$("body").on("click",".btn-remove-line", function()
		{
			var len = $(".proposal-div .btn-remove-line").length;
			if($(".proposal-div .btn-remove-line").length > 1)
			{
