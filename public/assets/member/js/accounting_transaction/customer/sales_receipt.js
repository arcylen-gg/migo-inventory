...
function sales_receipt()
{
	...
	function action_load_initialize_select()
	{
		$('.droplist-customer').globalDropList(
		{
			...
			{			
				...
				$(".count-open-transaction").html($(this).find("option:selected").attr("ctr-sr"));
	...
	function action_load_reference_number($customer_id, $data_value, $text_value)
	{
		...
		{
			$(".rcvpymnt-refno").val($(".droplist-customer").find('option:selected').attr('ctr-ref-num')); 
		}
		else
		{
	...
	function load_applied_transaction()
	{
		...
		$('.applied-pm').load('/member/transaction/sales_receipt/applied-pm', function()
		{
			action_load_initialize_select();
			number_input_change();
			event_remove_tr_pm();
