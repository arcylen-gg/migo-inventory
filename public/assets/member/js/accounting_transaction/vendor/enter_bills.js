...
function enter_bills()
{
	...
	function event_chenge_cost_rate() 
	{
		$("body").on("change",".new-cost", function()
		{
			...
			if($item_id && $item_id != 0 && $(".item-new-cost").val() == "new_cost")
			{
				...
				if($item_cost != $new_cost && $allow_item_new_cost == 'last_in_first_out')
				{
	...
	function action_load_initialize_select()
	{
		...
		{ 	
			...
			onChangeValue: function()
			{
				...
				$(".popup-link-open-transaction").attr('link','/member/transaction/enter_bills/load-transaction?vendor='+$(this).val());
				$(".count-open-transaction").html($(this).find("option:selected").attr("ctr-eb"));
			}
		});
	...
	function load_applied_transaction()
	{
		...
		$('.applied-transaction-list').load('/member/transaction/enter_bills/load-applied-transaction', function()
		{
			...
			$('.tax-eb').val($('.po-tax').val());
			$('.disc-type-eb').val($('.po-disc-type').val());
			$('.disc-percentage-eb').val($('.po-disc-percentage').val());
			action_compute();
