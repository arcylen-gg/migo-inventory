...
function wis_create()
{
	...
	function event_change_quantity()
	{
		$("body").on("keyup", ".quantity-item", function(e)
		{
			...
			$.ajax(
			{
				url 		: "/member/transaction/warehouse_transfer/change-quantity?item_id=" + item_id + "&qty=" + quantity,
				dataType 	: "json",
				type 		: "get",
	...
	function event_remote_item_from_cart()
	{
		$("body").on("click", ".remove-item-from-cart", function(e)
		{
			...
			$.ajax(
			{
				url:"/member/transaction/warehouse_transfer/create-remove-item",
				dataType:"json",
	...
	function action_scan_item($item_id)
	{
		...
		scandata._token = $(".token").val();
 		...
 		$.ajax(
		{
			url			: "/member/transaction/warehouse_transfer/scan-item",
			dataType	: "json",
			type 		: "post",
			data 		: scandata,
			success 	: function(data)
	...
	function action_load_item_table()
	{
		...
		$(".load-item-table-pos").load("/member/transaction/warehouse_transfer/table-item", function()
		{
